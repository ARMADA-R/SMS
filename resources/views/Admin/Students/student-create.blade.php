    ...
    <div class="col-11 /*col-lg-10*/ /*col-xl-8*/ mx-auto">
        ...
        <div class="my-4">
            ...
            <div class="card">
                ...
                <h5 class="card-title" style="line-height: 2;">{{trans('app.students.create-student')}}</h5> 
                                        
                </div>
                <div class="card-body">
                    <form role="form" action="{{ route('admin.student.create') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input required type="text" name="fname" id="fname" class="form-control" placeholder="Brown" />
                            </div>

                            <div class="form-group col-md">
                                <label for="lname">{{trans('app.students.lname')}}</label>
                                <input required type="text" name="lname" class="form-control" id="lname" placeholder="John" />
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md">
                                <label for="father_name">{{trans('app.students.father-name')}}</label>
                                <input required type="text" class="form-control" name="father_name" id="father_name" placeholder="Bard John" />
                            </div>
                            <div class="form-group col-md">
                                <label for="mother_name">{{trans('app.students.mother-name')}}</label>
                                <input required type="text" class="form-control" name="mother_name" id="mother_name" placeholder="Maria John" />
                            </div>
                            <div class="form-group col-md">
                                ...
                                <select required name="level" id="level" class="form-control">
                                    ...
                                    <option value="{{$level->id}}"> {{ $level->name .' : '. $level->name }} </option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <hr>
            <div class="card">
                <form role="form"  id="excel-form" method="POST" action="{{ route('admin.student.create.excel') }}">
                <div class="card-header">
                    <h5 class="card-title" style="line-height: 2;">{{trans('app.students.students-from-file')}}</h5>

                    <div class="card-tools">
                        <select required name="level" id="level" class="form-control">
                            <option value="" selected> --{{trans('app.levels.level')}}-- </option>
                            @foreach($levels as $level)
                            <option value="{{$level->id}}"> {{ $level->name .' : '. $level->name }} </option>
                            @endforeach
                        </select>
                ...
                <div class="card-body">
                        ...
                        <div class="form-group">
                            <div class="custom-file">
                                ...
                                <label class="custom-file-label" for="xlf">{{trans('app.files.upload-file')}}</label>
                            </div>
                            @csrf
                            <hr>
                            <div id="htmlout"></div>
                            <br>
                            <button type="button" onclick="ExcelSubmit();" class="btn btn-info">{{trans('app.save')}}</button>
                            <button type="reset" class="btn btn-outline-secondary">{{trans('app.reset')}}</button>
                            <input type="hidden" class="form-control" name="table" id="jstable" required>
                            <input type="submit" id="excel-form-submit" style="visibility: hidden;" value="">
                        </div>
                    </div>
                </form>
            </div>
